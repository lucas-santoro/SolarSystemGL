...
- **Procedural multi-layer starfield.** New self-contained `core/Starfield` renderer replaces the inline single-pass sky. The layered fragment shader places jittered sub-pixel star points (cube-face tiling, soft `fwidth`-anti-aliased splats) with power-law brightness, blackbody color temperature, a tilted Milky Way dust band, and subtle twinkle. A `Starfield` quality preset (Off / Low / Medium / High) in Settings selects how many layers shade — defaulted to Medium on touch devices as a perf lever.

...
Grid::Grid(Grid&& other) noexcept
    : VAO(other.VAO), VBO(other.VBO), lineCount(other.lineCount), lastBuiltStyle_(other.lastBuiltStyle_),
      lastBuiltResolution_(other.lastBuiltResolution_), lastBuiltExtent_(other.lastBuiltExtent_)
{
    other.VAO = 0;
    other.VBO = 0;
}
...
void Grid::buildCartesianGeometry(int divisions, float extent)
{
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, kFloatsPerVertex * sizeof(GLfloat), (void*) 0);
    glEnableVertexAttribArray(1);
    glVertexAttribPointer(1, 1, GL_FLOAT, GL_FALSE, kFloatsPerVertex * sizeof(GLfloat),
...
void Grid::buildRadialGeometry(int divisions, float extent)
{
    ...
    std::vector<GLfloat> vertices;
    vertices.reserve(static_cast<size_t>(rings) * kRingSegments * 2 * kFloatsPerVertex +
...
void Grid::rebuildIfDirty(const GridSettings& settings)
{
    ...
    if (!styleChanged && !resolutionChanged && !extentChanged)
        return;
...
void Grid::draw(Shader& shader, const std::vector<CelestialBody>& bodies, const GridSettings& settings)
{

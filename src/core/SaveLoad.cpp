                    ...
                    const PhysicsSystem& physics)
{
    ...
    for (const auto& b : bodies)
    {
        ...
        out << "atmosphereColor = " << b.atmosphereColor.r << " " << b.atmosphereColor.g << " "
            << b.atmosphereColor.b << "\n";
    ...
    while (std::getline(in, line))
    {
        ...
        else
        {
            if (key == "name")
                ...
                current->pos_m = parseDVec3(val);
            else if (key == "vel_m")
                current->vel_m = parseDVec3(val);
            else if (key == "mass_kg")
                current->mass_kg = std::stod(val);
            else if (key == "color")
                current->color = parseVec3(val);
            else if (key == "density")
                current->density = std::stof(val);
            else if (key == "emissive")
                current->emissive = std::stof(val);
            else if (key == "displayScale")
                current->displayScale = std::stof(val);
            else if (key == "hasRings")
                current->hasRings = (val == "1" || val == "true");
            else if (key == "hasAtmosphere")
                current->hasAtmosphere = (val == "1" || val == "true");
            else if (key == "atmosphereHeight")
                current->atmosphereHeight = std::stof(val);
            else if (key == "atmosphereColor")
                current->atmosphereColor = parseVec3(val);
            else if (key == "atmosphereDensity")
                current->atmosphereDensity = std::stof(val);

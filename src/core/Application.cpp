...
Application::Application(int windowWidth, int windowHeight, const char* title)
    : window_(windowWidth, windowHeight, title), camera_(kInitialCameraPosition),
      ...
      bloomCompositeShader_(embedded_shaders::FullscreenQuadVertexShader,
                            embedded_shaders::BloomCompositeFragmentShader),
      ...
      bloomPongFbo_(windowWidth / 2, windowHeight / 2, false), physics_(), uiManager_(),
      grid_(10000.0f, 200, 0.0f)
{
    ...
    const ImFont* interFont = io.Fonts->AddFontFromFileTTF("fonts/Inter-Regular.ttf", 16.0f);
...
void Application::handleTouchStart(const EmscriptenTouchEvent& e)
{
    ...
    int activeCount = 0;
    for (const TouchPoint& s : activeTouches_)
        if (s.id != -1)
            ++activeCount;
    if (activeCount == 2)
    {
...
void Application::handleTouchMove(const EmscriptenTouchEvent& e)
{
    ...
    for (int i = 0; i < e.numTouches; ++i)
    {
        ...
        for (size_t s = 0; s < activeTouches_.size(); ++s)
        {
            ...
            if (total > slot.maxMovementPx)
                slot.maxMovementPx = total;
...
void Application::tick()
{
    ...
    if (glfwGetKey(glfwWindow, GLFW_KEY_W) == GLFW_PRESS)
        camera_.processKeyboard(GLFW_KEY_W, deltaTime);
    if (glfwGetKey(glfwWindow, GLFW_KEY_S) == GLFW_PRESS)
        camera_.processKeyboard(GLFW_KEY_S, deltaTime);
    if (glfwGetKey(glfwWindow, GLFW_KEY_A) == GLFW_PRESS)
        camera_.processKeyboard(GLFW_KEY_A, deltaTime);
    if (glfwGetKey(glfwWindow, GLFW_KEY_D) == GLFW_PRESS)
        camera_.processKeyboard(GLFW_KEY_D, deltaTime);
    ...
    const bool predValid =
        uiManager_.showPathPrediction && predSelected >= 0 && predSelected < static_cast<int>(bodies_.size());
    if (predValid)
    {
        ...
        {
            pathPredictor_.recompute(bodies_, predSelected, predictionHorizonSeconds_,
                                     kPredictionSampleCount);
    ...
    const float aspectRatio =
        static_cast<float>(framebufferWidth) / static_cast<float>(std::max(framebufferHeight, 1));
    ...
    if (uiManager_.demoModeActive)
    {
        ...
        const glm::vec3 demoCameraPos(radius * std::cos(orbitAngle), radius * 0.45f,
    ...
    const glm::mat4 projection =
        glm::perspective(glm::radians(fieldOfView_), aspectRatio, kProjectionNearPlane, kProjectionFarPlane);
    ...
    if (uiManager_.showTrails)
        renderTrails(view, projection);
    if (uiManager_.showPathPrediction)
        renderPathPrediction(view, projection);
    if (uiManager_.showLagrange)
        renderLagrange(view, projection);
    if (uiManager_.isPlacingBody())
        renderPlacementHint(view, projection);
...
void Application::renderMenuFrame(float deltaTime)
{
    ...
    const glm::vec3 menuCameraPosition(kMenuOrbitRadius * std::cos(angle), kMenuOrbitHeight,
                                       kMenuOrbitRadius * std::sin(angle));

    const glm::mat4 view = glm::lookAt(menuCameraPosition, glm::vec3(0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    const glm::mat4 projection =
        glm::perspective(glm::radians(fieldOfView_), aspectRatio, kProjectionNearPlane, kProjectionFarPlane);
    ...
    case StartMenuAction::Quit:
        ...
        break;
    case StartMenuAction::Load: /* Not yet wired from the menu. */
        break;
    case StartMenuAction::Settings: /* Handled above. */
        break;
...
{
    if (slot < 0 || slot >= static_cast<int>(bookmarks_.size()))
        return;
...
void Application::loadDefaultBodies()
{
    ...
    const DefaultBody defaults[] = {
        // name      AU       v(m/s)    mass(kg)   density   color                                  emi  scale
        // rings  atm    height  atmColor                                  atmDensity
        {"Sun", 0.000, 0.0, 1.989e30, 1408.0f, glm::vec3(1.00f, 0.90f, 0.30f), 1.0f, 0.4f, false, false,
         0.00f, glm::vec3(0.0f), 0.0f},
        {"Mercury", 0.387, 47900.0, 3.3011e23, 5427.0f, glm::vec3(0.50f, 0.50f, 0.50f), 0.0f, 1.0f, false,
         false, 0.00f, glm::vec3(0.0f), 0.0f},
        {"Venus", 0.723, 35000.0, 4.8675e24, 5243.0f, glm::vec3(0.95f, 0.85f, 0.55f), 0.0f, 1.0f, false, true,
         0.08f, glm::vec3(1.00f, 0.85f, 0.40f), 1.5f},
        {"Earth", 1.000, 29780.0, 5.9720e24, 5514.0f, glm::vec3(0.20f, 0.40f, 1.00f), 0.0f, 1.0f, false, true,
         0.05f, glm::vec3(0.40f, 0.60f, 1.00f), 1.0f},
        {"Mars", 1.524, 24100.0, 6.4171e23, 3933.0f, glm::vec3(0.80f, 0.30f, 0.10f), 0.0f, 1.0f, false, true,
         0.03f, glm::vec3(0.95f, 0.50f, 0.35f), 0.5f},
        {"Jupiter", 5.203, 13070.0, 1.8980e27, 1326.0f, glm::vec3(0.90f, 0.70f, 0.40f), 0.0f, 1.0f, false,
         true, 0.08f, glm::vec3(0.95f, 0.78f, 0.50f), 1.4f},
        {"Saturn", 9.537, 9680.0, 5.6834e26, 687.0f, glm::vec3(0.95f, 0.85f, 0.50f), 0.0f, 1.0f, true, true,
         0.07f, glm::vec3(0.95f, 0.85f, 0.60f), 1.2f},
        {"Uranus", 19.191, 6800.0, 8.6810e25, 1271.0f, glm::vec3(0.60f, 0.85f, 0.90f), 0.0f, 1.0f, false,
         true, 0.06f, glm::vec3(0.45f, 0.85f, 0.90f), 1.1f},
        {"Neptune", 30.070, 5430.0, 1.0240e26, 1638.0f, glm::vec3(0.30f, 0.40f, 0.85f), 0.0f, 1.0f, false,
         true, 0.07f, glm::vec3(0.25f, 0.40f, 0.95f), 1.2f},
    };
...
void Application::createAuxiliaryBuffers()
{
    ...
    {
        const float angle =
            2.0f * 3.14159265358979f * static_cast<float>(segment) / static_cast<float>(kRingSegmentCount);
    ...
    glBufferData(GL_ARRAY_BUFFER, ringVertices.size() * sizeof(glm::vec3), ringVertices.data(),
                 GL_STATIC_DRAW);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(glm::vec3), nullptr);
    glEnableVertexAttribArray(0);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
...
void Application::destroyAuxiliaryBuffers()
{
    ...
    if (ringVAO_)
        glDeleteVertexArrays(1, &ringVAO_);
    if (ringVBO_)
        glDeleteBuffers(1, &ringVBO_);
...
void Application::refreshOrbitalCameraTarget()
{
    ...
    if (targetIndex < 0 || targetIndex >= static_cast<int>(bodies_.size()))
        return;
...
void Application::handleHotkeys()
{
    ...
    if (f1Down && !wasF1Pressed_)
        applyViewPreset(ViewPreset::Default);
    if (f2Down && !wasF2Pressed_)
        applyViewPreset(ViewPreset::TopDown);
    if (f3Down && !wasF3Pressed_)
        applyViewPreset(ViewPreset::SideOn);
    ...
    if (!keyboardCaptured)
    {
        if (spaceDown && !wasSpacePressed_)
            physics_.paused = !physics_.paused;
        if (rDown && !wasRPressed_)
            camera_.reset();
        ...
        if (twoDown && !wasTwoPressed_)
        {
            ...
            if (selected >= 0 && selected < static_cast<int>(bodies_.size()))
            {
                camera_.flyToOrbital(selected, bodies_[selected].renderPosition(),
                                     bodies_[selected].focusDistance());
            }
        }
    }
...
void Application::sampleOrbitTrails()
{
    for (auto& body : bodies_)
    {
        ...
        if (!body.trailPoints.empty())
        {
            ...
            if (jump > kTrailTeleportJumpWU)
                body.trailPoints.clear();
        }

        const bool firstSample = body.trailPoints.empty();
        const bool movedEnough =
            !firstSample && glm::distance(body.trailPoints.back(), currentPosition) > kMinimumTrailStepWU;
...
{
    const glm::vec3 lightPosition = bodies_.empty() ? glm::vec3(0.0f) : bodies_[0].renderPosition();
...
void Application::renderTrails(const glm::mat4& view, const glm::mat4& projection)
{
    ...
    for (const auto& body : bodies_)
    {
        if (body.trailPoints.size() < 2)
            continue;
        trailUploadBuffer.assign(body.trailPoints.begin(), body.trailPoints.end());
        glBufferData(GL_ARRAY_BUFFER, trailUploadBuffer.size() * sizeof(glm::vec3), trailUploadBuffer.data(),
                     GL_DYNAMIC_DRAW);
...
void Application::renderPlacementHint(const glm::mat4& view, const glm::mat4& projection)
{
    ...
    if (glm::length(endPos - startPos) < 0.1f)
        return; // no drag yet
...
void Application::renderLagrange(const glm::mat4& view, const glm::mat4& projection)
{
    ...
    if (selectedIdx <= 0 || selectedIdx >= static_cast<int>(bodies_.size()))
        return;
    ...
    if (primaryIdx == selectedIdx)
        return; // can't compute against itself
    ...
    const float separationWU =
        glm::length(bodies_[selectedIdx].renderPosition() - bodies_[primaryIdx].renderPosition());
...
void Application::renderPathPrediction(const glm::mat4& view, const glm::mat4& projection)
{
    ...
    glBindVertexArray(trailVAO_);
    glBindBuffer(GL_ARRAY_BUFFER, trailVBO_);
    glBufferData(GL_ARRAY_BUFFER, uploadBuffer.size() * sizeof(glm::vec3), uploadBuffer.data(),
                 GL_DYNAMIC_DRAW);

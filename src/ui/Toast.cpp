...
void ToastQueue::info(const std::string& message)    { push(message, kInfoColor); }
void ToastQueue::success(const std::string& message) { push(message, kSuccessColor); }
void ToastQueue::warning(const std::string& message) { push(message, kWarningColor); }
void ToastQueue::error(const std::string& message)   { push(message, kErrorColor); }
...
void ToastQueue::render()
{
    ...
    for (auto it = toasts_.rbegin(); it != toasts_.rend(); ++it, ++index)
    {
        ...
        const float  alphaScale = toast.remaining < kFadeOutWindow
                                      ? toast.remaining / kFadeOutWindow
                                      : 1.0f;

        const ImVec4 background(toast.color.r, toast.color.g, toast.color.b,
                                toast.color.a * alphaScale);

        ImGui::PushStyleColor(ImGuiCol_WindowBg, background);
        ImGui::PushStyleColor(ImGuiCol_Text,
                              ImVec4(0.05f, 0.05f, 0.05f, 1.0f * alphaScale));

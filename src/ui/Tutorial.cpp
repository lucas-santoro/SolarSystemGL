...
namespace
{
    ...
    constexpr Step kSteps[] = {
        {
            "Welcome to SolarSystemGL",
            "A Newtonian N-body simulator of our solar system, with realistic\n"
        ...
        {
            "Camera",
            "RIGHT-CLICK + DRAG  rotate the camera (mouse-look in Free mode,\n"
        ...
        {
            "Bodies & edit",
            "LEFT-CLICK a body to select it. The Planet Info panel drops down\n"
        ...
        {
            "Drag-to-place",
            "LEFT-CLICK + DRAG in empty space to spawn a new Earth-mass body\n"
        ...
        {
            "Time & saving",
            "TIME SCALE slider in the top bar controls simulated seconds per\n"
...
void Tutorial::render()
{
    ...
    if (ImGui::Button("Back", ImVec2(buttonWidth, 0.0f))) --step_;
    ImGui::EndDisabled();
    ImGui::SameLine();

    if (step_ + 1 < kStepCount)
    {
        if (ImGui::Button("Next", ImVec2(buttonWidth, 0.0f))) ++step_;
        ImGui::SameLine();

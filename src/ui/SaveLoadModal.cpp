...
namespace
{
    ...
    bool performSave(const std::string& path,
                     std::vector<CelestialBody>& bodies, PhysicsSystem& physics,
                     UIManager& uiManager)
    {
    ...
    void exportEntry(const std::string& path, const std::string& displayName,
                     UIManager& uiManager)
    {
...
void SaveLoadModal::gatherSaves()
{
    ...
    auto gatherFromDir = [](const std::string& directory, bool isBuiltIn,
    ...
    std::sort(userSaves_.begin(), userSaves_.end(),
              [](const SaveEntry& lhs, const SaveEntry& rhs)
...
std::string SaveLoadModal::sanitizeBasename(std::string name)
{
    ...
    if (name.empty()) name = "imported.txt";
    return name;
}
                           ...
                           Camera& camera, UIManager& uiManager)
{
    ...
    if (ImGui::BeginPopupModal(kSavePopupId, nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        {
            ImGui::Text("'%s' already exists. Overwrite?",
                        pendingOverwrite_->displayName.c_str());
            ImGui::Spacing();
    ...
    if (ImGui::BeginPopupModal(kLoadPopupId, nullptr, 0))
    {
        ...
        if (!userSaves_.empty())
        {
            ...
            for (size_t i = 0; i < userSaves_.size(); ++i)
            {
                ...
                if (ImGui::Selectable(e.displayName.c_str(), false,
        ...
        if (ImGui::BeginPopupModal("##confirm-delete", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
        {
            ImGui::Text("Delete '%s'? This cannot be undone.",
                        pendingDelete_->displayName.c_str());
            ImGui::Spacing();
